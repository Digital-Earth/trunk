...
{
    public class CoverageQuerier : IDisposable
    {
        ...
        public void Start(PyxNet.Stack stack, IList<String> searchTokens)
        {
            ...
            // TODO: Expose control of whether to clear providers, or not, to the client.
            // For now, emulate existing behaviour.
            lock (m_providers)
            {
                m_providers.Clear();
            }
        ...
        private void HandleResult(object sender, PyxNet.Querier.ResultEventArgs args)
        {
            ...
            if (null == queryResult.ExtraInfo ||
                queryResult.ExtraInfo.Identifier != PyxNet.Pyxis.CoverageRequestMessage.MessageID)
            {
                return;
            }

            ProcRef procRef;
            PyxNet.DataHandling.DataGuid id;
            int version;
            {
                PyxNet.Pyxis.CoverageRequestMessage extraInfo = 
                    new PyxNet.Pyxis.CoverageRequestMessage(queryResult.ExtraInfo);

                id = queryResult.MatchingDataSetID;
                version = extraInfo.ProcessVersion;

                procRef = new ProcRef(pyxlib.strToGuid(id.ToString()), version);
            }
            ...
            OnResultRaise(queryResult, procRef);
        }
    }
}
