...
namespace PyxNet.Utilities
{
    ...
    {
        #region Static
        public static TraceTool Trace = new TraceTool(false);
        ...
        {
            string xml = PyxNet.Utilities.XmlTool.ToXml(objectToWrite);
        ...
        public ReturnType Read<ReturnType>(Guid id) where ReturnType:class
        {
            ...
            try
            {
                ...
                if (adapter.Fill(image.Tables[0]) == 1)
                {
                    ...
                    return PyxNet.Utilities.XmlTool.FromXml<ReturnType>(xml);
                }
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<ReturnType> ReadAll<ReturnType>() where ReturnType : class
        {
            ...
            if (image != null)
            {
                foreach (DataRow r in image.Tables[0].Rows)
                {
                    ...
                    yield return PyxNet.Utilities.XmlTool.FromXml<ReturnType>(xml);
                }
            }
        }
    ...
    namespace Test
    {
        ...
        public class SimpleDatabaseTests
        {
            ...
            public void CreateDatabaseFromClassDefinitions()
            {
                ...
                using (PyxNet.Utilities.TemporaryFile dbFilename = new TemporaryFile())
                {
                    dbFilename.Delete();
