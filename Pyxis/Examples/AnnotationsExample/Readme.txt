...
This demo would show how to use the API in 4 steps:
...
4) Interacting with IconAnnotations using GlobeLayer.MouseAnnotation events


Files to Look at
----------------
2) Form1.cs - How to initialize a globe and Well data.
              Moreover, this would show you how to leverage GlobeLayer.MouseAnnotation events
...
the Pyxis.Core.Engine with the right setting for you.
...
Pyxis.UI.Layers.GlobeLayer.AnnotationMouse* events are a useful tool to create custom interaction with annotation
on top of the globe. Currently, GlobeLayer supports Icon Annotations.

Annotation Event arguments that are available using GlobeAnnotationMouseEventArgs are:
1) all System.Windows.Forms.MouseEventArgs.
2) FeaturesCount - how many features are annotated. sometimes icons reflect several features.
3) GetFeaturesIds() - allow the user enumerate all annotation features.
4) GeoSource - reference to which GeoSource generated those features.
5) ViewStateId - reference to the styled GeoSource id - this can be used to change styles of the GeoSource.