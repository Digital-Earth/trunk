app.controller('worldviewDetailsController', function ($scope, $pyx, $pyxconfig, $routeParams, $location, $timeout, $q, fileUploader, imageServer, wvAlerts, resource, resourceStatus, siteParams, detectAndLaunchStudio) {
    ...
    $scope.commitEditMode = function () {
        ...
        $pyx.gallery.update(newResource)
            .success(function () {
                wvAlerts.success($scope.resource.Type + ' updated successfully', $scope);
            ...
            error(function () {
                wvAlerts.error('Failed to update ' + $scope.resource.Type, $scope);
            });
    }
    ...
    $scope.commitDeleteMode = function () {
        ...
        $pyx.gallery.update(newResource)
            .success(function () {
                ...
                wvAlerts.success($scope.resource.Type + ' \'' +$scope.resource.Metadata.Name+ '\' was deleted successfully', $scope);
                $location.path(provider.Type + '/' + provider.Id);
            })
            .error(function (error) {
                wvAlerts.error('Failed to delete ' + $scope.resource.Type, $scope);
            });
    }
    ...
    $scope.uploadImage = function ($files) {
        ...
        .then(function (dataUrl) {
            ...
            return fileUploader.upload(imageServer.setImageUrl(), $scope.newImageFile, newName)
                .catch(function () { return wvAlerts.error('Failed to uploaded ' + $scope.resource.Type + ' image', $scope); });
        })
        .then(function(fileUpload) {
            return $pyx.gallery.update(newResource)
                .catch(function () { return wvAlerts.error('Failed to update ' + $scope.resource.Type, $scope); });
        })
        .then(function () {
            wvAlerts.success($scope.resource.Type + ' image updated successfully',$scope);
    ...
    $scope.beginSubscribe = function () {
        if (!$scope.canSubscribe) {
            return wvAlerts.error('Can\'t subscribe for this ' + $scope.resource.Type + ' at the moment');
        }
