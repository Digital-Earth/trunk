app.service("featureEditResources", function ($pyx, $filter, $timeout, $q, $pyxIntercom, worldViewStudioConfig, geoSourceCache) {
    var i18n = $filter("i18n");
    
    function register($scope) {
        $scope.addTemplate('dialogs', '/client/templates/studio/template/feature-edit-resources.html');
                ...
                $pyx.user.profile().success(function (profile) {
                    ...
                    geoSourcePromise.then(function (geoSource) {
                        ...
                        if (model.isOwner) {
                            ...
                            function updateStatus() {
                                if (!model.active){
                                    //no longer need to update the status
                                    return;
                                } else if (model.isLocalResource) {
                                    //local resouurce - keep check if that changed.
                                ...
                                $pyx(geoSource).status().then(function (result) {
                                    ...
                                    model.geoSourceStatus = undefined;
                                    model.statusRefreshTime++;
                                }).finally(function () {
                                    if (model.active) {
                                        $timeout(updateStatus, statusRefreshTimes[model.statusRefreshTime]);
            ...
            if ("capture" in $pyx.globe) {
                ...
                $scope.screenCapture.capture = function () {
                    if ($scope.screenCapture.onCapture) {
                        ...
                        $pyx.globe.capture(captureDetails).success(function (url) {
                            $timeout(function () {
