app.service("featurePropertiesWindow", function ($pyx, $timeout, $filter, $pyxIntercom, dispatcher, dispatcherPromise) {
    ...
    function register($scope,options) {
        $scope.addTemplate('dialogs', '/client/templates/studio/template/feature-properties-window.html');

        if (options && options.editStyle == true) {
            $scope.propWindowEditStyle = true;
        }
        ...
        $scope.$on("pyx-globe-ready", function () {
            ...
            if ('dragstart' in $pyx.globe) {
                $pyx.globe.dragstart(function() {
                   $timeout(function() {
                       $scope.showPropWindow = false;
                   });
                });
            ...
            $scope.handleGlobeRightClick = function (cursor, e) {                
                $scope.handleShowProperties(cursor,e,'forcePopup');
            };
            
            $scope.handleShowProperties = function (cursor, e, options) {
                if (document) {
