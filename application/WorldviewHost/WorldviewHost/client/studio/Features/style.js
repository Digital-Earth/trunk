app.service("featureStyle", function ($pyx, $pyxIntercom, dispatcher) {
    ...
    function randomPalette() {
        var result = $scope.styleOptions.palettes[nextPalette];
        nextPalette = (nextPalette + 1) % $scope.styleOptions.palettes.length;
        return result;
    }
    ...
    function register($scope) {
        ...
        $scope.styleModifier = {
            ...
            updatePaletteStep: function (item, step, newColor) {
                step.Color = newColor;
            ...
            toggleShowAsElevation: function (item) {
                ...
                $pyxIntercom.track(item.Style.ShowAsElevation ? "style-elevation" : "style-not-elevation");
                dispatcher.actions.updateItemStyle.invoke({ item: item });
                $scope.notifyLibraryChange();
            },
            ...
            updatePalette: function (item, palette, options) {
                ...
                if (item.Style.Icon && item.Style.Icon.PaletteExpression || options.field) {
                    dispatcher.actions.calculateItemStyle.invoke({
                        'item': item,
                        'field': options.field || item.Style.Icon.PaletteExpression,
                        'palette': palette,
                        'onScreen': options.useScreenBasedStyling
                    });
                }
                if (item.Style.Fill && item.Style.Fill.PaletteExpression || options.field) {
                    dispatcher.actions.calculateItemStyle.invoke({
                        'item': item,
                        'field': options.field || item.Style.Fill.PaletteExpression,
                        'palette': palette,
                        'onScreen': options.useScreenBasedStyling
                    });
                }
