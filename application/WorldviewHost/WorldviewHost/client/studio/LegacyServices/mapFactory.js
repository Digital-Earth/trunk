        ...
        load: function (mapDoc) {
            ...
            var map = {
                ...
                findItemGroup: function(item) {
                    var index = $pyx.array.firstIndex(mapDoc.Groups, function (group) {
                        var itemIndex = $pyx.array.firstIndex(group.Items, function (i) { return i.Resource.Id === item.Resource.Id; });
                        return itemIndex != -1;
                    });
                    if (index !== -1) {
                       return mapDoc.Groups[index];
                    }
                    return undefined;
                },
                removeItem: function (group, item) {
                    //if we only been supplied an item as the only argument
                    if (group.Resource && !item) {
                        item = group;
                        group = this.findItemGroup(item);
                    }
                ...
                dashboard: function (id) {
                    ...
                    var dashboard = {
                        ...
                        setSelection: function (geometry, color, borderColor) {
                            ...
                            var steppedRandom = function(min,max,step) {
                                            ...
                                            Effects: [
                                                {
                                                    ...
                                                    Properties: {
                                                        ...
                                                        Angle: steppedRandom(0,360,10),
                                                        //sine movement speed in length per sec
                                                        Speed: steppedRandom(0,1,0.2),
                                                        //the length of sine frequency in pixels
                                                        Length: steppedRandom(5,20,5)
                                                    }
                                                }]},
            ...
            angular.forEach(map.model.EmbeddedResources, function (resource) {
                ...
                dispatcher.actions.resourceResolvedCompleted.invoke({ 'resource': resource, 'local': true });
            });
