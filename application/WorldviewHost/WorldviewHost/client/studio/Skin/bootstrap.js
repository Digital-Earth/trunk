...
app.service("worldViewStudioBootstrap", function (
    ...
    return function ($scope, bootOptions) {
        ...
        // keep the production console clean (disable console.log())
        if (window.location.hostname === "pyxisglobe.com" || window.location.hostname === "www.pyxisglobe.com") {
        ...
        */
        ...
        $scope.theme = {
            url: "" //for example: '/assets/Images/Studio/Themes/calgary2.jpg'
        }
        ...
        $scope.templates = {}
        $scope.excludedTemplates = $scope.excludedTemplates || [];

        $scope.addTemplate = function (section, url) {
            if ($scope.excludedTemplates.indexOf(url) > -1) {
                return;
            }
        ...
        switch (bootOptions.mode) {
            ...
            case 'viewer':
                ...
                featureLibrary.register($scope, { useLocalStorage: false });
                featureAuthentication.register($scope, { forceLogin: false });
                break;

            case 'default':
            default:
                featurePropertiesWindow.register($scope, { editStyle: true });
        ...
        $scope.$on("studio-setup-started", function () {
            $pyx.globe.ready(function () {
                ...
                $pyx.globe.click(function (index, e, metadata) {
                    $timeout(function () {
                        $scope.handleGlobeClick(index, e, metadata);
                        $scope.$emit("pyx-globe-click", { $event: e, $index: index, metadata: metadata });
                    });
                });

                $pyx.globe.rightClick(function (index, e, metadata) {
                    $timeout(function () {
                        $scope.handleGlobeRightClick(index, e, metadata);
                        $scope.$emit("pyx-globe-right-click", { $event: e, $index: index, metadata: metadata });
                    });
                });
