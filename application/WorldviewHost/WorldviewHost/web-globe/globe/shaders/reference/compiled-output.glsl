...
/// FragmentState definitions ///
//TODO: maybe move to ThreeBoostrap module?
...
/// CotangentFrame fragment only definitions ///
...
/// value-to-normal-geosource-8be6a2ec-5110-49cf-a295-1008f8e9a21b fragment only definitions ///

vec3 valueToNormal_1(vec2 uv) {
    vec3 normal = normalize( vNormal );
    vec2 fuvdx = dFdx( uv );
    vec2 fuvdy = dFdy( uv );
    
    float elevationScale = 20.0;
    
    float s0 = sampleValueNormalized_1(uv);
    float s1 = sampleValueNormalized_1(uv + fuvdy);
    float s2 = sampleValueNormalized_1(uv + fuvdx);
    float s3 = sampleValueNormalized_1(uv - fuvdy);
    float s4 = sampleValueNormalized_1(uv - fuvdx);
...
float sampleValue_2(vec2 uv) {
   vec2 uv2 = uv * uvTrans_1.xy + uvTrans_1.zw;
   return sampleValue_1(uv2);
}

vec3 valueToNormalTransUV_1(vec2 uv) {
   vec2 uv2 = uv * uvTrans_1.xy + uvTrans_1.zw;
   return valueToNormal_1(uv2);
}


/// elevation-geosource-8be6a2ec-5110-49cf-a295-1008f8e9a21b fragment only definitions ///

void elevation_1(out vec4[8] _fragment_state) {
    vec3 normal = valueToNormalTransUV_1(vUv);
    float elevation_1 = sampleValue_2(vUv);
    _fragment_state[1].xyz = normal;
    _fragment_state[7].x = elevation_1;
}
...
vec4 gradient_1(vec2 uv) {
    ...
    vec4 color = vec4(0.0);
    color += mix(vec4(0.0,0.0,0.0,1.0),vec4(1.0,1.0,1.0,1.0),clamp((value - 0.0)/(7000.0 - 0.0),0.0,1.0));
    return color;
}
...
void phong_1(out vec4[8] _fragment_state) {
    ...
    float shininess = _fragment_state[6].y;
    float specularStrength = _fragment_state[6].z;
...
#endif
    _fragment_state[0].xyz = mix(_fragment_state[0].xyz,totalDiffuse,_fragment_state[6].x) + totalSpecular;
}
...
vec4 _fragment_state[8];
_fragment_state[0] = vec4(0.5,0.5,0.5,1.0);
_fragment_state[1].xyz = normalize(vNormal);
_fragment_state[1].w = 1.0;
_fragment_state[2] = vec4(0.0);
_fragment_state[3] = vec4(lightingParams.x);
_fragment_state[4] = vec4(lightingParams.z);
_fragment_state[5] = vec4(0.0);
_fragment_state[6].y = lightingParams.y;
_fragment_state[6].z = 1.0;
_fragment_state[7].x = 0.0;


/// elevation-geosource-8be6a2ec-5110-49cf-a295-1008f8e9a21b code ///
elevation_1(_fragment_state);

/// color-undefined code ///
vec4 color_1 = gradient_1(vUv);
_fragment_state[0].rgb = mix(_fragment_state[0].rgb, color_1.rgb, 1.0);

/// Phong code ///
phong_1(_fragment_state);
