...
*/
app.service('featureSearch', function ($pyx, $filter, $timeout, $rootScope, $pyxIntercom,
    worldViewStudioConfig, mapFactory, searchServices, connectionIndex, onlineGeospatialService, localSearch, mapSearchService, geocodeService, networkServiceStateAlert) {
    ...
    function register($scope) {
        $scope.gallerySearch = searchServices.registerGallerySearch(
            ...
            function (error) {
                ...
                networkServiceStateAlert('gallerySearch', false, i18n('Failed to connect to WorldView Gallery'));
                $scope.gallerySearch.results = [];
            });
        ...
        $scope.handleSearchAction = function (action, resource) {
            ...
            case "performWhere":
                ...
                $pyxIntercom.track('filter-selection-on-property-value');
                break;
        ...
        $scope.addExpression = function (expression) {
            ...
            $q.all(promises).then(function () {
                $pyx.when($pyx.engine.calculate(geoSources, expression.Expression)).then(function (newGeoSource) {
                    $scope.currentMap.importGeoSource(newGeoSource).success(function (style) {
