app.service("featureSelectionTools", function ($pyx, $filter, $q, $timeout, $pyxIntercom, styleOptions, dispatcher) {
    ...
    function register($scope) {
        $scope.addTemplate('sections', '/template/feature-selection-tools-sections.html');
        $scope.addTemplate('dialogs', '/template/feature-selection-tools-dialogs.html');
        ...
        $scope.$on("pyx-globe-ready", function () {
            $scope.handleGlobeClick = function (cursor, e) {
                if (document) {
        ...
        $scope.$on("pyx-globe-keyup", function ($event) {
