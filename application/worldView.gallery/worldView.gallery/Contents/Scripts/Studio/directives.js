...
<div class="nav-tools" onmousedown="return false">\
    <div class="compass-button" ng-mousedown="startRotation($event)">\
        ...
        <div class="tilt-button" ng-click="tiltGlobe()" stop-propagation="mousedown" tooltip="{{\'tooltip.navigation.tilt\'|i18n}}"></div>\
    </div>\
...
app.directive('searchResultMenu', function ($parse, connectionIndex, worldViewStudioConfig) {
    return {
        restrict: "E",
        templateUrl: "/template/search-result-menu.html",
        scope: {
            data: "=",
            itemSelector: "=",
            itemImporter: "=",
...
//require: /template/search-result-resource.html
//
//attributes:
...
app.directive('searchResultResource', function ($pyx, $parse, $timeout, $filter, $pyxIntercom, connectionIndex, networkServiceStateAlert, worldViewStudioConfig) {
    return {
        restrict: "E",
        templateUrl: "/template/search-result-resource.html",
        scope: {
            resource: "=",
            currentMap: "=",
            parentSelection: "=",
...
//require: /template/library-item-legend.html
//
//attributes:
...
app.directive('libraryItemLegend', function ($pyx, $filter, paletteService, geoSourceCache) {
    return {
        restrict: "E",
        templateUrl: "/template/library-item-legend.html",
        scope: {
            item: "=",
            styleModifier: "=",
            copyAndPaste: "="
...
<svg ng-attr-width="{{svgWidth}}" ng-attr-height="{{svgHeight}}">\
    ...
    <rect ng-repeat="data in dataset track by $index"\
        ...
        ng-mouseenter="onEnter(data,$index,$event)"\
        ng-mouseleave="onLeave(data,$index,$event)">\
    </rect>\
</svg>',
        link: function (scope, element, attr) {
            ...
            scope.onEnter = function (data, index, event) {
                if (scope.onBarEnter) {
                    scope.onBarEnter({ $data: data, $index: index, $event: event });
                }
            }

            scope.onLeave = function (data, index, event) {
                if (scope.onBarLeave) {
                    scope.onBarLeave({ $data: data, $index: index, $event: event });
                }
            }
...
//
app.directive('backgroundPalette', function () {
    return {
        restrict: 'A',
        link: function (scope, element, attr) {
            ...
            if (attr.valueTransform) {
                ...
                valuePositionFunction = function (value, index, count) {
                    fn(scope, { $value: value, $index: index, $count: count });
                }
            }
...
// by the widget.
app.directive('widget', function ($timeout, $pyx, $filter, mapFactory, geoSourceCache) {
    return {
        restrict: 'E',
        scope: {
            model: '='
...
<div class="grid-item" row-span="model.Settings.RowSpan" col-span="model.Settings.ColSpan">\
    ...
    <div ng-if="widgetState.State==\'error\'">\
        ...
        link: function (scope, element, attr, dashboardCtrl, worldViewStudioConfig) {
...
app.directive("colorPicker", function (styleOptions) {
    return {
        ...
        template: '<div class="color-picker" ng-style="{\'background-color\':selectedColor}" ng-class="{\'is-transparent\': selectedColor == \'rgba(0,0,0,0.0)\' || selectedColor == \'rgba(0,0,0,0.00)\'}" popup-template="/popup-menu/color-picker.html" placement="left center" popup-group="popup-menu" show-trigger="click"></div>',
        link: function (scope, element, attrs) {
   ...
   */
    function postGradientDesignerLink(scope, element, attrs, ctrl) {
        ...
        scope.appendStep = function () {
            var newValue, fetchStep, stepIndex, stepColor;
            var newStep = {};
            var delayApply;
        ...
        scope.updateStep = function (oldValue, step, transform) {
            $timeout(function () {
                var newStep, nextStep, prevStep;
                var stepCount = scope.getNumStep() - 1;
        ...
        var setupEditor = function () {
            var elStops = element.find('.gradient-legend-stop');
    ...
    return {
        ...
        templateUrl: '/template/gradient-designer.html',
    ...
    */
    function gradientStopLink(scope, element, attrs) {
        ...
        var makeDraggable = function () {
            ...
            var elStopSwatch = elStopHandle.find('.gradient-stop-swatch');
...
app.directive("searchBoxTag", function ($timeout, $pyxIntercom, dispatcher) {
    return {
        ...
        replace: true,
        template: ['<div class="search-box-tag" ',
            'popup-template="/popup-menu/search-tag-menu.html" popup-group="popup-menu" placement="bottom center" show-trigger="click" append-to-body="true" hide-delay="1000">',
