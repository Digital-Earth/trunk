...
namespace FileScope
{
	...
	//public class SearchTabPage : UserControl
	{
		...
		public SearchTabPage(string search, bool deserialize)
		{
			...
			else
			{
				if(search.Length < 8 || search.Substring(0, 8) != "browse: ")
				{
                    ...
                    FileScopeCore.Gnutella2.Search.BeginSearch(search);
				}
				else
                    FileScopeCore.Gnutella2.Sck.OutgoingBrowseHost(search.Substring(8, search.Length - 8));
			}
		}
		...
		public void AddNewItem(QueryHitObject elQho, QueryHitTable elQht)
		{
			...
			// temporarily commented out to allow duplicate search results
            //if(elQht == null)
            //{
		...
		void HighlightItem(ListViewItem lvi)
		{
			...
			*/
			if(tbl.downloaded)
				lvi.ForeColor = StartApp.settings.clListBoxFore;
			else if(score >= 4)
				...
				lvi.ForeColor = StartApp.settings.clHighlight4;
		}
		...
		void Stop()
		{
			lock(FileScopeCore.Gnutella2.Search.activeSearches)
                foreach (FileScopeCore.Gnutella2.Search g2s in FileScopeCore.Gnutella2.Search.activeSearches)
		...
		void Close()
		{
			...
			//remove this search
            lock (FileScopeCore.Gnutella2.Search.activeSearches)
                for (int y = 0; y < FileScopeCore.Gnutella2.Search.activeSearches.Count; y++)
				{
                    FileScopeCore.Gnutella2.Search g2s = (FileScopeCore.Gnutella2.Search)FileScopeCore.Gnutella2.Search.activeSearches[y];
					if(g2s.query == this.Tag.ToString())
					{
						g2s.TerminateSearch();
                        FileScopeCore.Gnutella2.Search.activeSearches.RemoveAt(y);
                        if (FileScopeCore.Gnutella2.Search.activeSearches.Count == 0)
                            FileScopeCore.Gnutella2.Search.hubsWithoutKeys.Clear();
						break;
					}
				}
			lock(ActiveSearch.searches)
		...
		void Download()
		{
			...
			for(int y = 0; y < listResults.SelectedItems.Count; y++)
			{
				...
				if(elqho.fileSize > 0)
				{
					...
					if(StartApp.settings.fileAlert)
					{
