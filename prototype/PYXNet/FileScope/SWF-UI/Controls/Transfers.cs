...
namespace FileScope
{
	...
	public class Transfers : System.Windows.Forms.UserControl
	{
		...
		public Transfers()
		{
			...
			Themes.SetupTheme(c);
            this.updownUploads.Value = Configuration.settings.maxUploads;
		}
		...
		private void tmr1_Tick(object sender, System.EventArgs e)
		{
			try
			{
				...
				toolBar2.Buttons[0].Text = "Downloads (" + Configuration.State.downloadsNow.ToString() + ")";
				toolBar1.Buttons[0].Text = "Uploads (" + Configuration.State.uploadsNow.ToString() + ")";
			}
			catch
			{
		...
		public void NewDownload(QueryHitTable qht, int dlNum)
		{
			...
			if(StartApp.settings.switchTransfers && Configuration.State.opened)
		...
		public void RemoveUpload(int upNum, int type)
		{
			foreach(ListViewItem lvi in listViewUps.Items)
				if(lvi.SubItems[5].Text == upNum.ToString())
				{
					if(type == 0 || StartApp.settings.clearUp)
		...
		void DownCancel(bool delete)
		{
			...
			if(StartApp.settings.cancelDLAlert)
			{
		...
		void updownStuff()
		{
            ...
            Configuration.settings.maxUploads = (int)updownUploads.Value;
		}
		...
		private void Transfers_Resize(object sender, System.EventArgs e)
		{
			try
			{
				panel1.Height = (int)(this.ClientSize.Height * StartApp.settings.transSplitPerc);
			}
			catch
			{
		...
		{
			StartApp.settings.transSplitPerc = (float)panel1.Height / this.ClientSize.Height;
		}
	}
}
