	...
	public class TransfersMoreInfoDlg : System.Windows.Forms.Form
	{
		...
		void UpdateUploadInfo(int fIndex)
		{
			if(fIndex >= 0 && label1.Text == "Name: ")
			{
				...
				{
					label1.Text += System.IO.Path.GetFileName(((FileObject)Configuration.fileList[fIndex]).location);
					label2.Text += Utils.Assemble(((FileObject)Configuration.fileList[fIndex]).b, " bytes");
					label3.Text += ((FileObject)Configuration.fileList[fIndex]).sha1;
					if(((FileObject)Configuration.fileList[fIndex]).md4 != null)
						label6.Text += Utils.HexGuid(((FileObject)Configuration.fileList[fIndex]).md4);
				}
			}
		}
