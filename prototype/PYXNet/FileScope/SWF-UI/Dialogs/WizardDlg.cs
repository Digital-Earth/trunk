	...
	public class WizardDlg : System.Windows.Forms.Form
	{
		...
		private void button7_Click(object sender, System.EventArgs e)
		{
			...
			if(radioDialup.Checked)
			{
                ...
                Configuration.settings.maxUploads = 2;
			}
			else if(radioCable.Checked)
			{
                Configuration.settings.connectionType = EnumConnectionType.cable;
                Configuration.settings.maxUploads = 10;
			}
			else if(radioT1.Checked)
			{
                Configuration.settings.connectionType = EnumConnectionType.t1;
                Configuration.settings.maxUploads = 20;
			}
			else
			{
                Configuration.settings.connectionType = EnumConnectionType.t3;
                Configuration.settings.maxUploads = 30;
			}
			//not ultrapeer by default
            Configuration.settings.ultrapeerCapable = Configuration.State.Gnutella2Stats.ultrapeer  = false;
			//send all of the shares to Configuration.State.shareList
			Configuration.shareList.AddRange(listShares.Items);
			//download directory
            Configuration.settings.dlDirectory = textDownload.Text;
			//we don't know our public IP yet
			System.Net.IPHostEntry ipEntry = System.Net.Dns.GetHostEntry(System.Net.Dns.GetHostName());
            Configuration.settings.ipAddress = ipEntry.AddressList[0].ToString();
			//get permanent GUID
            Configuration.settings.myGUID = GUID.newGUID();
			//initial connections to keep when ultrapeer
            Configuration.settings.gConnectionsToKeep = 200;
			//behind firewall?
            Configuration.settings.firewall = chkFirewall.Checked;
			//enable filescope update notifications
			StartApp.settings.updateNotify = true;
			//not always on top
			StartApp.settings.alwaysOnTop = false;
			//switch to transfers view on a new download
			StartApp.settings.switchTransfers = true;
			//minimizing and closing
			StartApp.settings.minimizeNormal = false;
			StartApp.settings.closeNormal = true;
			...
			StartApp.settings.fileAlert = true;
			//alert by default when cancelling downloads
			StartApp.settings.cancelDLAlert = true;
			//don't clear transfers by default
			StartApp.settings.clearDl = false;
			StartApp.settings.clearUp = false;
			//main window settings
			StartApp.settings.mainHeight = 0;
			StartApp.settings.mainMax = false;
			StartApp.settings.mainWidth = 0;
			//good splitter percentage
			StartApp.settings.transSplitPerc = .6F;
			//setup colors
			...
			Configuration.SaveBlockedChatHosts();
			//web caches
