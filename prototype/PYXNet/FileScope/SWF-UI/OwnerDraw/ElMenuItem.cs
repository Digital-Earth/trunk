...
namespace FileScope
{
	...
	public class ElMenuItem : MenuItem
	{
		...
		void SetupBrushes()
		{
			...
			hovBrush = new SolidBrush(StartApp.settings.clMenuHighlight1);
			if(selBrush != null) selBrush.Dispose();
			selBrush = new SolidBrush(StartApp.settings.clMenuHighlight2);
			if(borderPen != null) borderPen.Dispose();
			borderPen = new Pen(StartApp.settings.clMenuBorder);
			if(boxBrush != null) boxBrush.Dispose();
			boxBrush = new SolidBrush(StartApp.settings.clMenuBox);
			if(backBrush != null) backBrush.Dispose();
			backBrush = new SolidBrush(StartApp.settings.clFormsBack);
		...
		protected override void OnDrawItem(DrawItemEventArgs e)
		{
			...
			else
			{
				...
				{
					textBrush.Dispose();
					textBrush = new SolidBrush(StartApp.settings.clMenuBox);
