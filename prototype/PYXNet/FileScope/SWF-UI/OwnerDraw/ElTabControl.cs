...
namespace FileScope
{
	...
	public class ElTabControl : TabControl
	{
		...
		public void ResetBrushes()
		{
			...
			bgBrush = new SolidBrush(StartApp.settings.clFormsBack);
			if(hb != null) hb.Dispose();
			hb = new HatchBrush(HatchStyle.ZigZag, StartApp.settings.clLabelFore2, Color.Transparent);
			...
			strBrush = new SolidBrush(StartApp.settings.clLabelFore);
			if(highlightBrush != null) highlightBrush.Dispose();
			highlightBrush = new SolidBrush(StartApp.settings.clMenuHighlight1);
			...
			borderPen = new Pen(StartApp.settings.clMenuBorder, 1);
		}
		...
		void PaintItem(int x, PaintEventArgs e, int index)
		{
			...
			if(index == activeTab)
				...
				strBrush = new SolidBrush(Color.FromArgb(220, StartApp.settings.clLabelFore));
