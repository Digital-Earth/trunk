    ...
    public class Chat
    {
        ...
        public void Outgoing(ref string peer)
        {
            try
            {
                ...
                Configuration.guiBridge.NewChat(chatNum);
        ...
        public void Incoming(Socket elSock)
        {
            try
            {
                ...
                if (Configuration.blockedChatHosts.Contains(this.RemoteIP()))
                {
                    Disconnect();
                    return;
                }
        ...
        public void Disconnect()
        {
            try
            {
                ...
                Configuration.guiBridge.DisconnectedChat(chatNum);
                state = ChatState.Closed;
                connectYet.Stop();
                buf.Clear();
                if (sock1 != null)
                ...
                ChatManager.nullifyMe nm = new ChatManager.nullifyMe(ChatManager.NullifyMe);
                nm(this.chatNum);
            }
            catch
            {
