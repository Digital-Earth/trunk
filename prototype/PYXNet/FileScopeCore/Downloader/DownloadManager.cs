...
namespace FileScopeCore
{
    ...
    public class DownloadManager : IDeserializationCallback
    {
        ...
        public static void NewDownload(QueryHitTable qht)
        {
            ...
            Configuration.State.downloadsNow++;

            //update gui
            Configuration.guiBridge.NewDownload(qht, index);
        ...
        void DownloadDone()
        {
            lock (endpoints)
            {
                ...
                try
                {
                    if (!Directory.Exists(Configuration.settings.dlDirectory))
                    {
                        Directory.CreateDirectory(Utils.GetCurrentPath("downloads"));
                        Configuration.settings.dlDirectory = Utils.GetCurrentPath("downloads");
                    }
                ...
                //update gui
                Configuration.guiBridge.RemoveDownload(dmNum);
                ...
                DownloadManager.nullifyDMerAndDLer nm = new DownloadManager.nullifyDMerAndDLer(DownloadManager.NullifyDMerAndDLer);
                nm(dmNum);
            }
        }
        ...
        public void CancelDownload(bool deleteAfter)
        {
            lock (endpoints)
            {
                ...
                try
                {
                    ...
                    Directory.CreateDirectory(Path.Combine(Configuration.settings.dlDirectory, "incomplete"));
        ...
        public string GetFile(string overdir)
        {
            ...
            if (Directory.Exists(Configuration.settings.dlDirectory))
            {
                eldir = Configuration.settings.dlDirectory;
            }
            else
            {
        ...
        void tmrUpdate_Tick(object sender, ElapsedEventArgs e)
        {
            ...
            if (respawnDLers)
            {
                try
                {
                    ...
                    //update gui
                    Configuration.guiBridge.NewDownload(tempqht, this.dmNum);
                }
            ...
            Configuration.guiBridge.UpdateDownload(ref status, ref percent, ref speed, dmNum);
        }
