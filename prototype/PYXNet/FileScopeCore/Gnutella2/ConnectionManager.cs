...
namespace FileScopeCore.Gnutella2
{
    ...
    public class ConnectionManager
    {
        ...
        public static void SwitchMode()
        {
            ...
            Configuration.State.Gnutella2Stats.ultrapeer = false;
            OQH2.ResetStaticPart();

            //inform gui
            Configuration.guiBridge.SwitchedMode();
        }
        ...
        {
            Configuration.State.Gnutella2Stats.lastConnectionCount = ultrapeers.Count + leaves.Count;
            int goodCount;
            if (Configuration.State.Gnutella2Stats.ultrapeer)
            {
        ...
        static void SpawnConnections(int num)
        {
            ...
            if (HostCache.recentHubs.Count > 0)
            {
                for (int x = 0; x < num; x++)
                {
                    //CEH added try-catch to get around null oject reference running in debug
                    try
                    {
                        IPAddress tmpIPA;
                        string host;
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine("Connection Manager:SpawnConnections: " + e.Message.ToString());
                    }
                }
            }
            else
            {
                try
                {
                    if (Configuration.gnutella2WebCache.Count == 0)
                    {
                        //CEH comment out for now
                        //System.Diagnostics.Debug.WriteLine("no more webcache servers");
                        return;
                    }
                    for (int x = 0; x < 4; x++)
                    {
                        string cacheServer = (string)Configuration.gnutella2WebCache[GUID.rand.Next(0, Configuration.gnutella2WebCache.Count)];
