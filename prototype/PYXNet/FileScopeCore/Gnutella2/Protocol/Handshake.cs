    ...
    public class Handshake
    {
            ...
            for (; ; )
            {
                ...
                else if (lineLower.IndexOf("remote-ip:") != -1)
                {
                    if (!Handshake.ipSet)
                    {
                        ...
                        {
                            Configuration.settings.ipAddress = matchIP.ToString();
                            Configuration.myIPA = System.Net.IPAddress.Parse(Configuration.settings.ipAddress);
                ...
                else if (lineLower.IndexOf("accept-encoding: deflate") != -1)
                {
                    ...
                    if (Configuration.State.Gnutella2Stats.ultrapeer)
                    {
                        ...
                        {
                            if (Configuration.State.Gnutella2Stats.enableDeflate)
                            {
                                sck.deflateOut = true;
                            }
                        }
            ...
            lock (sck)
            {
                ...
                if (sck.state != Condition.hndshk3)
                {
                    ...
                    if (sck.state == Condition.hndshk)
                    {
                        ...
                        if (sck.mode == G2Mode.Leaf && ConnectionManager.leaves.Count >= Configuration.settings.gConnectionsToKeep)
                        {
                            busy = true;
                        }
                    }
                    if (sck.state == Condition.hndshk2 && Configuration.State.Gnutella2Stats.ultrapeer && !sck.ultrapeerNeeded)
                    {
        ...
        public static OMessage Outgoing(Sck sck, bool busy)
        {
            ...
            if (sck.state == Condition.Connected)
            {
                ...
                hm.handshake += "Listen-IP: " + Configuration.settings.ipAddress + ":" + Configuration.settings.port.ToString() + "\r\n";
                hm.handshake += (remoteIP == "") ? "" : "Remote-IP: " + remoteIP + "\r\n";
                hm.handshake += "User-Agent: " + Configuration.userAgent + " " + Configuration.version + "\r\n";
                ...
                hm.handshake += "X-Ultrapeer: " + ((Configuration.State.Gnutella2Stats.ultrapeer == true) ? "True\r\n" : "False\r\n");
                hm.handshake += "\r\n";
            }
