            ...
            if (this.sockNum != -1)
            {
                ...
                if (!relay)
                {
                    ...
                    if (udp != null)
                    {
                        ...
                        if (root.be != !Configuration.State.le)
                        {
    ...
    public class KnownHubList : Message
    {
        ...
        public class NeighborHub : Child
        {
            public override void Process(int offset, int len)
            {
                ...
                lock (HostCache.recentHubs)
                {
                    IPEndPoint neighbourHub = new IPEndPoint(Endian.GetIPAddress(root.inPayload, offset), Endian.ToUInt16(root.inPayload, offset + 4, root.be));

                    // Add it to the neighbours of the socket
                    Sck.scks[root.sockNum].neighbors.Add(neighbourHub);
/* trying to get hubs to propogate - but wasn't great -- w.i.p. when abandoned
                    // add it to the Hub Cache
                    HubInfo hubInfo = new HubInfo();
                    hubInfo.port = (UInt16)neighbourHub.Port;
                    // new hubs have been known for no time
                    hubInfo.timeKnown = 0;
                    HostCache.AddRecentAndCache(neighbourHub.Address, hubInfo);
*/                    
                }
            }
    ...
    public class Push : Message
    {
        ...
        public override void Process(int offset, int len)
        {
            ...
            do
            {
                if (offsetGUID != -1)
                {
                    if (GUID.Compare(root.inPayload, offsetGUID, Configuration.settings.myGUID, 0))
                    {
                        //this push was for us
                        break;
                    }
                    else if (Configuration.State.Gnutella2Stats.ultrapeer)
                    {
    ...
    public class QueryKeyAnswer : Message
    {
        ...
        public override void Process(int offset, int len)
        {
            ...
            {
                //routing?
                if (Configuration.State.Gnutella2Stats.ultrapeer && !this.sna.Equals(Configuration.myIPA) && this.qna == null)
                {
                    lock (HostCache.recentHubs)
                    {
    ...
    public class Query : Message
    {
        ...
        public override void Process(int offset, int len)
        {
            ...
            //check query key
            bool sendACK = false;
            if (Configuration.State.Gnutella2Stats.ultrapeer && this.ipepReturn != null && this.fromUdp)
            {
            ...
            if (Configuration.State.Gnutella2Stats.ultrapeer && !this.fromUdp)
            {
            ...
            if (dn.Length > 0)
            {
                Configuration.guiBridge.QueryG2(ref dn);
            }
            G2Word g2words = null;
            ...
            if (dn.Length > 0 || urns != null)
            {
                ...
                if (this.urns != null)
                {
                    ...
                    {
                        foreach (FileObject fi in Configuration.fileList)
                        {
                            bool match = false;
                            curURN = this.urns;
                            while (curURN != null)
                ...
                if (matches != null)
                {
                    ...
                    //if(this.ipepReturn != null && (Configuration.State.Gnutella2Stats.ultrapeer || (Configuration.State.everIncoming && Configuration.State.udpIncoming)))
    ...
    public class QueryHit : Message
    {
        ...
        public override void Process(int offset, int len)
        {
                ...
                if (Sck.scks[root.sockNum].browseHost)
                {
                    ...
                    if (this.qhos != null)
                    {
                        foreach (QueryHitObject qho in this.qhos)
                        {
                            ...
                            Configuration.guiBridge.AddQueryHit(qho, null, ref searchname);
                        }
                    }
                    return;
                }
                        ...
                        if (GUID.Compare(search.guid, 0, root.inPayload, offset + 1))
                        {
                            ...
                            if (this.qhos != null)
                            {
                                foreach (QueryHitObject qho in this.qhos)
                                {
                                    ...
                                    qho.speed = 50;
                                    qho.vendor = this.vendor;
                                    Configuration.guiBridge.AddQueryHit(qho, null, ref search.query);
                                }
                            }
                            return;
                        }
            ...
            //qh2 isn't for us
            while (Configuration.State.Gnutella2Stats.ultrapeer)
            {
            ...
            if (Configuration.State.downloadsNow == 0)
            {
                return;
            }
        ...
        public class VendorCode : Child
        {
            public override void Process(int offset, int len)
            {
                if (len == 4)
                {
                    ((QueryHit)root).vendor = Gnutella2.Vendor.GetVendor(Encoding.ASCII.GetString(root.inPayload, offset, 4));
                }
            }
        }
