...
namespace FileScopeCore.Gnutella2
{
    /*
        ...
        public override bool FillSendBuff(Sck sck, ref int buffIndex)
        {
            lock (pckt)
            {
                if (pckt.Length == 0)
                {
                    ...
                    pckt[17] = (byte)'G';
                    pckt[18] = (byte)'U';
                    Array.Copy(Configuration.settings.myGUID, 0, pckt, 19, 16);
                ...
                if (Configuration.State.Gnutella2Stats.ultrapeer)
                {
                    pcktLen = pckt.Length;
                    //library status
                    uint filesTotal = (uint)Configuration.State.filesShared;
                    uint kbTotal = (uint)Configuration.State.kbShared;
                    ...
                    Array.Copy(Endian.GetBytes(filesTotal, !Configuration.State.le), 0, pckt, 46, 4);
                    Array.Copy(Endian.GetBytes(kbTotal, !Configuration.State.le), 0, pckt, 50, 4);
                    //hub status
                    Array.Copy(Endian.GetBytes((ushort)ConnectionManager.leaves.Count, !Configuration.State.le), 0, pckt, 58, 2);
                    Array.Copy(Endian.GetBytes((ushort)Configuration.settings.gConnectionsToKeep, !Configuration.State.le), 0, pckt, 60, 2);
                }
                else
                {
                    pcktLen = pckt.Length - 8;
                    //library status
                    Array.Copy(Endian.GetBytes(Configuration.State.filesShared, !Configuration.State.le), 0, pckt, 46, 4);
                    Array.Copy(Endian.GetBytes(Configuration.State.kbShared, !Configuration.State.le), 0, pckt, 50, 4);
                }
                Endian.VarBytesFromInt(pckt, 1, pcktLen - 5, 1);
                Array.Copy(Endian.BigEndianIP(Configuration.settings.ipAddress), 0, pckt, 9, 4);
                Array.Copy(Endian.GetBytes((ushort)Configuration.settings.port, !Configuration.State.le), 0, pckt, 13, 2);
    ...
    public class OKHL : OMessage
    {
        ...
        public override bool FillSendBuff(Sck sck, ref int buffIndex)
        {
            lock (HostCache.recentHubs)
            {
                ...
                buffIndex++;
                Array.Copy(Endian.GetBytes(Configuration.State.timestamp, !Configuration.State.le), 0, sck.sendBuff, buffIndex, 4);
                buffIndex += 4;
                ...
                for (int x = 0; x < ConnectionManager.ultrapeers.Count; x++)
                {
                    ...
                    buffIndex += 4;
                    Array.Copy(Endian.GetBytes((ushort)Sck.scks[((ConnectionManager.G2Host)ConnectionManager.ultrapeers.GetKey(x)).sockNum].port, !Configuration.State.le), 0, sck.sendBuff, buffIndex, 2);
                    buffIndex += 2;
                }
                ...
                while (this.chCount > 0)
                {
                    ...
                    buffIndex += 4;
                    Array.Copy(Endian.GetBytes(((HubInfo)HostCache.recentHubs.GetByIndex(pos)).port, !Configuration.State.le), 0, sck.sendBuff, buffIndex, 2);
                    buffIndex += 2;
                    Array.Copy(Endian.GetBytes(Configuration.State.timestamp - ((HubInfo)HostCache.recentHubs.GetByIndex(pos)).timeKnown, !Configuration.State.le), 0, sck.sendBuff, buffIndex, 4);
                    buffIndex += 4;
                    pos++;
    ...
    public class OQHT : OMessage
    {
        ...
        public override bool FillSendBuff(Sck sck, ref int buffIndex)
        {
            if (reset)
            {
                ...
                pckt[4] = (byte)'T';
                pckt[5] = 0x00;
                Array.Copy(Endian.GetBytes(QueryRouteTable.ourQHT.Length, !Configuration.State.le), 0, pckt, 6, 4);
                pckt[10] = 0x01;
        ...
        public void SetupUdpPacket(OutgoingPacket op)
        {
            ...
            if (Configuration.State.udpIncoming)
            {
                if (OQKR.localIP == null)
                    OQKR.localIP = Endian.BigEndianIP(Configuration.settings.ipAddress);
                Array.Copy(OQKR.localIP, 0, udpPckt, 18, 4);
                Array.Copy(Endian.GetBytes((ushort)Configuration.settings.port, !Configuration.State.le), 0, udpPckt, 22, 2);
            }
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        Array.Copy(Endian.GetBytes((ushort)Sck.scks[sockNum].port, !Configuration.State.le), 0, udpPckt, 22, 2);
                    }
                }
            }
            op.deflate = false;
    ...
    public class OQKA : OMessage, IUdpMessage
    {
        ...
        void FillRest(byte[] payload, ref int buffIndex)
        {
            ...
            if (this.qk)
            {
                ...
                buffIndex++;
                Array.Copy(Endian.GetBytes(queryKey, !Configuration.State.le), 0, payload, buffIndex, 4);
                buffIndex += 4;
            }
            //sna
            Setllen(payload, buffIndex, 1);
        ...
        void CreatePacket(int i)
        {
            if (pckt == null)
            {
                ...
                if (!tcpQuery)
                {
                    ...
                    {
                        Array.Copy(Endian.GetBytes(Configuration.myIPA), 0, pckt, i, 4);
                        i += 4;
                        Array.Copy(Endian.GetBytes((ushort)Configuration.settings.port, !Configuration.State.le), 0, pckt, i, 2);
                        i += 2;
                    }
                    else
                    {
                        ...
                        i += 4;
                        Array.Copy(Endian.GetBytes((ushort)proxyPort, !Configuration.State.le), 0, pckt, i, 2);
                        i += 2;
                    }
                    Array.Copy(Endian.GetBytes(hi.mhi.queryKey, !Configuration.State.le), 0, pckt, i, 4);
                    i += 4;
                }
                //urn
                if (this.urn != null)
    ...
    public class OQA : OMessage, IUdpMessage
    {
        ...
        void FillBuff(byte[] bytarr, ref int buffIndex)
        {
            ...
            buffIndex++;
            Array.Copy(Endian.GetBytes(Configuration.State.timestamp, !Configuration.State.le), 0, bytarr, buffIndex, 4);
            buffIndex += 4;
            ...
            for (int x = 0; x <= ConnectionManager.ultrapeers.Count; x++)
            {
                ...
                if (x < ConnectionManager.ultrapeers.Count)
                {
                    ...
                    buffIndex += 4;
                    Array.Copy(Endian.GetBytes((ushort)Sck.scks[tmpSckNum].port, !Configuration.State.le), 0, bytarr, buffIndex, 2);
                    buffIndex += 2;
                    Array.Copy(Endian.GetBytes(Sck.scks[tmpSckNum].leaves, !Configuration.State.le), 0, bytarr, buffIndex, 2);
                    buffIndex += 2;
                }
                else
                {
                    Array.Copy(Endian.GetBytes(Configuration.myIPA), 0, bytarr, buffIndex, 4);
                    buffIndex += 4;
                    Array.Copy(Endian.GetBytes((ushort)Configuration.settings.port, !Configuration.State.le), 0, bytarr, buffIndex, 2);
                    buffIndex += 2;
                    Array.Copy(Endian.GetBytes((ushort)ConnectionManager.leaves.Count, !Configuration.State.le), 0, bytarr, buffIndex, 2);
                    buffIndex += 2;
                }
            }
            ...
            foreach (ConnectionManager.G2Host g2h in ConnectionManager.ultrapeers.Keys)
                if (Sck.scks[g2h.sockNum].neighbors.Count > 0)
                    foreach (IPEndPoint ipep in Sck.scks[g2h.sockNum].neighbors)
                    {
                        ...
                        buffIndex += 4;
                        Array.Copy(Endian.GetBytes((ushort)ipep.Port, !Configuration.State.le), 0, bytarr, buffIndex, 2);
                        buffIndex += 2;
                    }
            int pos = GUID.rand.Next(0, HostCache.recentHubs.Count);
            while (this.sCount2 > 0)
            {
                ...
                buffIndex += 4;
                Array.Copy(Endian.GetBytes(((HubInfo)HostCache.recentHubs.GetByIndex(pos)).port, !Configuration.State.le), 0, bytarr, buffIndex, 2);
                buffIndex += 2;
                Array.Copy(Endian.GetBytes(Configuration.State.timestamp - ((HubInfo)HostCache.recentHubs.GetByIndex(pos)).timeKnown, !Configuration.State.le), 0, bytarr, buffIndex, 4);
                buffIndex += 4;
                pos++;
                if (pos == HostCache.recentHubs.Count)
                    pos = 0;
    ...
    public class OQH2 : OMessage, IUdpMessage
    {
        ...
        void FillStaticPart()
        {
            lock (pcktStaticPart)
            {
                ...
                int sockNum = -1;
                if (!Configuration.State.Gnutella2Stats.ultrapeer && ConnectionManager.ultrapeers.Count > 0)
                {
                ...
                i++;
                Array.Copy(Configuration.settings.myGUID, 0, pcktStaticPart, i, 16);
                i += 16;
                //NA
                Setllen(pcktStaticPart, i, 1);
                Setnlen(pcktStaticPart, i, 2);
                ...
                i++;
                Array.Copy(Endian.GetBytes(Configuration.myIPA), 0, pcktStaticPart, i, 4);
                i += 4;
                Array.Copy(Endian.GetBytes((ushort)Configuration.settings.port, !Configuration.State.le), 0, pcktStaticPart, i, 2);
                i += 2;
                //NH
                if (sockNum != -1)
                {
                    ...
                    i += 4;
                    Array.Copy(Endian.GetBytes((ushort)Sck.scks[sockNum].port, !Configuration.State.le), 0, pcktStaticPart, i, 2);
                    i += 2;
                }
                //V
                Setllen(pcktStaticPart, i, 1);
        ...
        void FillDynamicPart(byte[] bytarr, ref int i)
        {
            foreach (FileObject fo in this.matches)
            {
                ...
                if (interests.all || interests.descriptivenames)
                {
                    ...
                    bytarr[i] = (byte)'Z';
                    i++;
                    Array.Copy(Endian.GetBytes(fo.b, !Configuration.State.le), 0, bytarr, i, 4);
                    i += 4;
                    //dn
    ...
    public class OPush : OMessage, IUdpMessage
    {
        ...
        public static void SendPush(QueryHitObject qho)
        {
            if (qho.servIdent == null)
                return;
            if (!Configuration.State.Gnutella2Stats.ultrapeer && !Configuration.State.udpIncoming && !Configuration.State.everIncoming)
                return;
        ...
        void CreatePacket()
        {
            if (pckt.Length != 0)
            {
                Array.Copy(this.gitem.m_guid, this.gitem.loc, pckt, 10, 16);
                return;
            }
            ...
            pckt[i] = (byte)'O';
            i++;
            Array.Copy(this.gitem.m_guid, this.gitem.loc, pckt, i, 16);
            i += 16;
