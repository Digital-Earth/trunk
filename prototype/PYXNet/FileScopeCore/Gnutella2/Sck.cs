    ...
    public class Sck
    {
        ...
        public void Reset(string addr)
        {
            try
            {
                lock (this)
                {
                    ...
                    if (!this.browseHost)
                    {
                        Configuration.guiBridge.G2NewConnection(addr, this.sockNum);
                    }

                    //CEH - had to use this obsolete method - for my ip 60. ...
                    IPHostEntry IPHost = Dns.GetHostByAddress(address);
        ...
        public void ResetIncoming(Socket tmpSock, byte[] bytesMsg)
        {
            try
            {
                lock (this)
                {
                    ...
                    if (!browseHost)
                    {
                        Configuration.guiBridge.G2NewConnection(RemoteIP(), this.sockNum);
                    }
                    connband.Start();
                    connectYet.Start();
                }
                ...
                if (!Configuration.AllowMultiConnections && !browseHost)
                {
                ...
                lock (this)
                {
                    ...
                    else
                    {
                        ...
                        oqh2.matches = Configuration.fileList;
                        oqh2.BrowseHostData(out dataQH2s);

                        //create the initial response
                        string resp = "HTTP/1.1 200 OK\r\n";
                        resp += "Server: " + Configuration.userAgent + " " + Configuration.version + "\r\n";
                        ...
                        //CEH debug
                        System.Diagnostics.Debug.WriteLine("browse host data: " + Encoding.ASCII.GetString(dataQH2s));
        ...
        public void ResetWebCache(string server)
        {
            try
            {
                lock (this)
                {
                    ...
                    Configuration.guiBridge.G2NewConnection(server, this.sockNum);
                    connectYet.Start();
        ...
        void FuncWebCache()
        {
            try
            {
                ...
                if (wcUpdate)
                {
                    httpRequest = (HttpWebRequest)WebRequest.Create(address + "?client=" + Configuration.userAgentCode + Configuration.version.Substring(0, 3) + "&net=gnutella2&update=1&ip=" + Configuration.myIPA.ToString() + @"%3A" + Configuration.settings.port.ToString());
                }
                else
                {
                    httpRequest = (HttpWebRequest)WebRequest.Create(address + "?client=" + Configuration.userAgentCode + Configuration.version.Substring(0, 3) + "&net=gnutella2&get=1");
                }
                httpRequest.UserAgent = Configuration.userAgent;
        ...
        void OnGetResponse(IAsyncResult ar)
        {
                ...
                else
                {
                    ...
                    while (host != null)
                    {
                        ...
                        else if (host.Length > 9 && host[0] == 'u' && host[1] == '|' && !hostAdded)
                        {
                            ...
                            {
                                if (!Configuration.gnutella2WebCache.Contains(strwc) && Configuration.gnutella2WebCache.Count < 60)
                                {
                                    Configuration.gnutella2WebCache.Add(strwc);
                                    hostAdded = true;
                                }
                            }
                        }
            ...
            catch (Exception e)
            {
                ...
                if (remove || !removedWebCacheToday)
                {
                    lock (Configuration.gnutella2WebCache)
                    {
                        for (int posgwc = 0; posgwc < Configuration.gnutella2WebCache.Count; posgwc++)
                        {
                            if ((string)Configuration.gnutella2WebCache[posgwc] == this.address)
                            {
                                System.Diagnostics.Debug.WriteLine("gwc2 removed: " + this.address);
                                Configuration.gnutella2WebCache.RemoveAt(posgwc);
            ...
            try
            {
                lock (this)
                {
                    ...
                    else
                    {
                        ...
                        req += "User-Agent: " + Configuration.userAgent + Configuration.version + "\r\n";
        ...
        /// <summary>
        /// This method is used by FuncSend() to generate the buffer index.
        /// </summary>
        private int FuncSendHelper()
        {
            //keep track of index in the buffer
            int buffIndex = 0;

            for (ArrayList alBuf = oBuf1; ; )
            {
                lock (alBuf)
                {
                    for (int x = 0; x < alBuf.Count; x++)
                    {
                        ...
                        try
                        {
                            if (!pckt.FillSendBuff(this, ref buffIndex))
                            {
                                return buffIndex;
                            }
                            alBuf.RemoveAt(0);
        ...
        public void SendData(byte[] byt, int loc, int len)
        {
            try
            {
                ...
                if (browseHost)
                {
                    Configuration.State.outTransferBandwidth += len;
                }
                else
                {
        ...
        public void Disconnect(string why)
        {
            ...
            {
#if DEBUG
                if (state == Condition.hndshk3 || state == Condition.hndshk2 || state == Condition.hndshk)
                {
                    System.Diagnostics.Debug.WriteLine("--Disconnected--" + state.ToString() + "--" + sockNum.ToString() + "--" + why);
                }
#endif
                if (state == Condition.Closed)
                {
                ...
                if (!browseHost && !Configuration.State.Gnutella2Stats.ultrapeer)
                {
                ...
                try
                {
                    if (webCacheThread != null)
                    {
                        if (webCacheThread.IsAlive) webCacheThread.Abort();
                    }
                }
                ...
                try
                {
                    if (!Configuration.State.closing && !this.browseHost)
                    {
                        Configuration.guiBridge.G2JustDisconnected(this.sockNum);
                    }
                    connectYet.Stop();
                    connband.Stop();
                    deflateIn = false;
        ...
        public void JustFinishedHandshake()
        {
            ...
            lock (HostCache.recentHubs)
            {
                ...
                Configuration.guiBridge.G2JustConnected(this.sockNum);
                this.addedHost = true;
            }
            lock (this)
            {
                ...
                if (!Configuration.State.udpIncoming && !Configuration.State.Gnutella2Stats.ultrapeer && this.mode == G2Mode.Ultrapeer)
                {
                    OUdpPing oup = new OUdpPing();
                    Array.Copy(Endian.GetBytes(Configuration.myIPA), 0, oup.endpoint, 0, 4);
                    Array.Copy(Endian.GetBytes((ushort)Configuration.settings.port, !Configuration.State.le), 0, oup.endpoint, 4, 2);
                    SendPacket(oup);
                }
                else
                {
        ...
        void connband_Tick(object sender, ElapsedEventArgs e)
        {
            ...
            Configuration.guiBridge.G2Update(this.sockNum, ref kbInVal, ref kbOutVal);
        }
    }
}
