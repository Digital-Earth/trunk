...
namespace FileScopeCore.Gnutella2
{
    ...
    public class Ultrapeer
    {
        ...
        {
            //CEH force ultrapeerCapable
            if (Configuration.settings.allowUltrapeer)
            {
                metBandwidthRequirement = true;
                return;
            }

            if (Configuration.State.outTransferBandwidth + Configuration.State.inTransferBandwidth > minBandwidth)
            {
                metBandwidthRequirement = true;
            }
        }
        ...
        public static void tmrUpCheck_Tick(object sender, ElapsedEventArgs e)
        {
            ...
            minMinutes--;
            if (minMinutes <= 0 && metBandwidthRequirement && Configuration.State.everIncoming && Configuration.State.udpIncoming)
            {
            ...
            if (minWCupdate <= 0)
            {
                if (ConnectionManager.ultrapeers.Count > 3 && ConnectionManager.leaves.Count > 30 && Configuration.gnutella2WebCache.Count > 0)
                {
                    Sck.OutgoingWebCache((string)Configuration.gnutella2WebCache[GUID.rand.Next(0, Configuration.gnutella2WebCache.Count)]);
                }
                minWCupdate = 68;
            }
#if DEBUG
