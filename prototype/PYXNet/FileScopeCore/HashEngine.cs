    ...
    public class HashEngine
    {
        ...
        static void FuncThread()
        {
            while (true)
            {
                try
                {
                    if (fileListIndex >= Configuration.fileList.Count)
                    {
                        Thread.Sleep(1000);
                        return;
                    }
                    ...
                    {
                        FileObject fo = (FileObject)Configuration.fileList[fileListIndex];
                    ...
                    if (Configuration.lastFileSet.ContainsKey(filePathName))
                    {
                        FileObject2 fo2 = (FileObject2)Configuration.lastFileSet[filePathName];
                    ...
                    lock (Configuration.fileList)
                    {
                        ...
                        Configuration.fileList[fileListIndex] = fo;
                    }
                    fileListIndex++;
                    Thread.Sleep(200);
                }
                ...
                {
                    Configuration.hashEngineAborted = true;
