...
namespace FileScopeCore
{
    ...
    public class UploadManager
    {
        ...
        public static void Outgoing(int fileIndex, string ip, int port)
        {
            ...
            if (fileIndex >= Configuration.fileList.Count || (fileIndex < 0 && fileIndex != -5))
            {
                return;
            }
        ...
        public static void NullifyMe(int upNum)
        {
            if (ups[upNum] != null)
            {
                try
                {
                    if (ups[upNum].sendThread != null)
                    {
                        if (ups[upNum].sendThread.IsAlive)
                        {
                            ups[upNum].sendThread.Abort();
                        }
                    }
                }
                catch
