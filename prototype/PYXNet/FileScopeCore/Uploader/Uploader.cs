    ...
    public class Uploader
    {
        ...
        public void Incoming(int upNum, Socket tmpSock, ref string elMsg)
        {
            try
            {
                ...
                //stats
                Configuration.State.uploadsNow++;
                active = true;

                //update gui
                Configuration.guiBridge.NewUpload(upNum);
        ...
        {
            bool match = false;
            lock (Configuration.fileList)
            {
                foreach (FileObject fo in Configuration.fileList)
                {
                    if (fo.md4 != null)
                    {
        ...
        void OnConnect(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (this.fileIndex == -5)
                {
                    ...
                    strMsg += Utils.HexGuid(Configuration.settings.myGUID) + "\r\n\r\n";
                ...
                else
                {
                    ...
                    strMsg += Utils.HexGuid(Configuration.settings.myGUID) + "/";
                    FileObject fo = (FileObject)Configuration.fileList[fileIndex];
        ...
        void FuncThread()
        {
            ...
            {
                //only for Gnutella
                if ((request.ToLower().IndexOf(@"get /") == 0 || request == ""))
                {
                ...
                //send update to gui
                Configuration.guiBridge.UpdateUpload(Path.GetFileName(fileName), totalSize, "Uploading", "0%", start.ToString() + " - " + stop.ToString(), "0 KB/s", upNum);
                ...
                if (response != "") // && this.edsck == null)
                {
                    ...
                    Configuration.State.outTransferBandwidth += response.Length;
                }
                ...
                while (bytes_to_send > 0)
                {
                    if (bytes_to_send > 4096)
                    {
                        ...
                        sock1.Send(readBuf);
                        Configuration.State.outTransferBandwidth += 4096;
                    ...
                    else
                    {
                        ...
                        Configuration.State.outTransferBandwidth += (int)bytes_to_send;
        ...
        {
            //update truespeed
            double kbTrue = ((double)Configuration.State.inTransferBandwidth / (double)1024) + ((double)Configuration.State.outTransferBandwidth / (double)1024);
            if ((int)kbTrue > Configuration.State.GetSpeed() && (int)kbTrue > 1)
            {
                Configuration.State.trueSpeed = (int)kbTrue;
            ...
            //update
            Configuration.guiBridge.UpdateUpload("", totalSize, "Uploading (" + ipStored + ")", percent, ((uint)(total_to_send - bytes_to_send + start)).ToString() + "-" + stop.ToString(), speed, upNum);
        }
        ...
        public void StopEverything(string where)
        {
            ...
            {
                Configuration.guiBridge.RemoveUpload(upNum, 0);
            }
            else if (ready && bytes_to_send == 0)
            {
                Configuration.guiBridge.RemoveUpload(upNum, 1);
            }
            else
            {
                Configuration.guiBridge.RemoveUpload(upNum, 2);
            }
            ...
            UploadManager.nullifyMe nm = new UploadManager.nullifyMe(UploadManager.NullifyMe);
            nm(upNum);
        }
        ...
        string CreateResponse()
        {
            ...
            if (fileName != "" || elSha1 != "")
            {
                lock (Configuration.fileList)
                {
                    foreach (FileObject fo in Configuration.fileList)
                    {
                        if (elSha1 != "" && elSha1.ToLower() == fo.sha1.ToLower())
                        {
                            ...
                            try
                            {
                                actualfsize = (uint)inf.Length;
                            }
                            catch
                            {
                                System.Diagnostics.Debug.WriteLine("CreateResponse filesize check");
                            }
            ...
            if (Configuration.State.uploadsNow > Configuration.settings.maxUploads)
            {
                fileName = "al;skdfj;asldfj";
            ...
            resp += "Chat: " + Configuration.settings.ipAddress + ":" + Configuration.settings.port.ToString() + "\r\n";
