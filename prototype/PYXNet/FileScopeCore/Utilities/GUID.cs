...
namespace FileScopeCore
{
    ...
    public class GUIDitem
    {
		...
		public GUIDitem(byte[] guid) : this(guid, 0)
		{
		}
        ...
        public override bool Equals(object obj)
        {
            ...
            for (int x = 0; x < 16; x++)
            {
                if (this.m_guid[this.loc + x] != gitem.m_guid[gitem.loc + x])
                {
                    return false;
                }
            }
        ...
        public bool Equals(byte[] guid)
        {
            for (int x = 0; x < 16; x++)
            {
                if (this.m_guid[this.loc + x] != guid[x])
                {
                    return false;
                }
            }
    ...
    public class GUID
    {
        ...
        public class GuidComparer : System.Collections.IComparer
        {
            public int Compare(object gUid1, object gUid2)
            {
                ...
                for (int x = 0; x < 16; x++)
                {
                    int diff = gitem1.m_guid[gitem1.loc + x] - gitem2.m_guid[gitem2.loc + x];
                    if (diff != 0)
                    {
                        return diff;
                    }
