...
function StoreIntoTree(level, limit)
    local store = {}
...
function CompressDuplicateZeroes(level, limit)
    local store = {}
    local nextPointer = nextPointerFun()    
    limit = limit or math.huge
    collectgarbage("collect")
...
repeat
    ...
    for level= minLevel, maxLevel  do
        result[limit][level] = {StoreIntoTree(level, limit), CompressDuplicateZeroes(level, limit)}
    end 
until limit > #indexes
...
for _,v in ipairs(limits)  do   
   ...
   h2 = h2..string.format('%10s,%10s,', "all", "s:0's")
end 
...
print('"Note: S:0 => multiple zeroes are stored in a new index"')
