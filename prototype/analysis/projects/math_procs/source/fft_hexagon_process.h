#ifndef FFT_HEXAGON_FILTER
#define FFT_HEXAGON_FILTER
/******************************************************************************
...
/*!
PYXFFTHexagonFilter, is a wrapper filter for the PYXFourierTransformHexagon. 
...
class PYXFFTHexagonFilter : public FilterImpl
{
public:
	...
	//! Default constructor.
	PYXFFTHexagonFilter(const PYXIcosIndex& index,
		int nRadius);

	//! Destructor.
	virtual ~PYXFFTHexagonFilter();
	...
	//! Get the attribute value at the specified PYXIS index.
	virtual PYXValue getCoverageValue(	const PYXIcosIndex& index,
										int nAttributeIndex = 0) const;
...
private:
	...
	*/
	...
	//! Checks and assigns the radius throws exception if invalid radius.
	void assignRadius(int nRadius);

protected:

	//! Disable copy constructor
	PYXFFTHexagonFilter(const PYXFFTHexagonFilter&);

	//! Disable copy assignment
	void operator =(const PYXFFTHexagonFilter&);

private:

	//! FFT Transform object, created when the filter is created. 
	PYXPointer<PYXFourierTransformHexagon> m_spTransform; 
