...
void RhombusBitmapColorizer::GrayScaleColorizer::colorPixel(const PYXValue & value,unsigned char * rgba) const
{
	if (m_useInt)
	{
		...
		unsigned char color = clamp(255*val/m_intFactor);
		memset(rgba,color,3);
		rgba[3] = m_alpha;
	}
	else
	{
		...
		unsigned char color = clamp((int)(val*m_doubleFactor));
		memset(rgba,color,3);
		rgba[3] = m_alpha;
	}
}
...
///////////////////////////////////////////////////////////////////////
// RGBFloat32ConstAlphaColorizer::RGBFloat32ConstAlphaColorizer
///////////////////////////////////////////////////////////////////////

RhombusBitmapColorizer::RGBFloat32ConstAlphaColorizer::RGBFloat32ConstAlphaColorizer(float min, float max, unsigned char alpha)
	: m_alpha(alpha), m_min(min), m_range(max-min)
{
}

unsigned char RhombusBitmapColorizer::RGBFloat32ConstAlphaColorizer::transform(float number) const
{
	auto fixed = (number - m_min) / m_range;
	if (fixed < 0) return 0;
	if (fixed > 1) return 255;
	return static_cast<int>(fixed * 255);
}

void RhombusBitmapColorizer::RGBFloat32ConstAlphaColorizer::colorPixel(const PYXValue & value, unsigned char * rgba) const
{
	const float * buffer = value.getFloatPtr(0);
	rgba[0] = transform(buffer[0]);
	rgba[1] = transform(buffer[1]);
	rgba[2] = transform(buffer[2]);
	rgba[3] = m_alpha;
}
