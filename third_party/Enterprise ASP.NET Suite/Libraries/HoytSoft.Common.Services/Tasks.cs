...
namespace HoytSoft.Common.Services {
	public sealed class DirectTask<T> {
		#region Variables
		private DirectTaskResult operationResult;
	...
	public class InstallServiceTask : ServiceTask {
		...
		private ServiceStartType serviceStartType;
		private ServiceErrorControl serviceErrorControl;
		...
		protected override TaskResult runServiceTask() {
			...
			try {
				...
				if (!string.IsNullOrEmpty(description)) {
					...
					try {
						Native.SERVICE_DESCRIPTION info = new Native.SERVICE_DESCRIPTION();
