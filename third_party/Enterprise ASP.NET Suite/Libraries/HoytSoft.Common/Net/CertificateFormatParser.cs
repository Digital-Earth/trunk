...
namespace HoytSoft.Common.Security {
	#region Enums
	...
	public class CertificateFormatParser {
		#region Enums
		private enum PEMPart {
			Unknown			= 0,
			OpenSSLPrivate	= 1, 
			OpenSSLPublic	= 2, 
			PKCS8			= 3,
		...
		private const String
			HEADER_PEM_CERTIFICATE		= "-----BEGIN CERTIFICATE-----",
			FOOTER_PEM_CERTIFICATE		= "-----END CERTIFICATE-----",
			HEADER_PEM_PRIVATE			= "-----BEGIN RSA PRIVATE KEY-----",
			FOOTER_PEM_PRIVATE			= "-----END RSA PRIVATE KEY-----",
			HEADER_PEM_PUBLIC			= "-----BEGIN PUBLIC KEY-----",
			FOOTER_PEM_PUBLIC			= "-----END PUBLIC KEY-----",
			HEADER_PEM_PKCS8			= "-----BEGIN PRIVATE KEY-----",
			FOOTER_PEM_PKCS8			= "-----END PRIVATE KEY-----",
			HEADER_PEM_PKCS8_ENCRYPTED	= "-----BEGIN ENCRYPTED PRIVATE KEY-----",
			FOOTER_PEM_PKCS8_ENCRYPTED	= "-----END ENCRYPTED PRIVATE KEY-----"
		;
		#endregion
		...
		#region The Meat
		private static byte[] translatePart(string contents, SecureString password, out PEMPart part, out int endIndex, out string newContent) {
			string innerContent = string.Empty;
			string header = string.Empty;
			string footer = string.Empty;
			newContent = contents;
			part = PEMPart.Unknown;
			endIndex = -1;
			...
			if (contents.StartsWith(HEADER_PEM_PRIVATE) && (endIndex = contents.IndexOf(FOOTER_PEM_PRIVATE)) >= 0) {
				part = PEMPart.OpenSSLPrivate;
				header = HEADER_PEM_PRIVATE;
				footer = FOOTER_PEM_PRIVATE;
			} else if (contents.StartsWith(HEADER_PEM_PUBLIC) && (endIndex = contents.IndexOf(FOOTER_PEM_PUBLIC)) >= 0) {
				part = PEMPart.OpenSSLPublic;
				header = HEADER_PEM_PUBLIC;
				footer = FOOTER_PEM_PUBLIC;
			} else if (contents.StartsWith(HEADER_PEM_PKCS8) && (endIndex = contents.IndexOf(FOOTER_PEM_PKCS8)) >= 0) {
				part = PEMPart.PKCS8;
				header = HEADER_PEM_PKCS8;
				footer = FOOTER_PEM_PKCS8;
			} else if (contents.StartsWith(HEADER_PEM_PKCS8_ENCRYPTED) && (endIndex = contents.IndexOf(FOOTER_PEM_PKCS8_ENCRYPTED)) >= 0) {
				part = PEMPart.PKCS8Encrypted;
				header = HEADER_PEM_PKCS8_ENCRYPTED;
				footer = FOOTER_PEM_PKCS8_ENCRYPTED;
			} else if (contents.StartsWith(HEADER_PEM_CERTIFICATE) && (endIndex = contents.IndexOf(FOOTER_PEM_CERTIFICATE)) >= 0) {
				part = PEMPart.Certificate;
				header = HEADER_PEM_CERTIFICATE;
				footer = FOOTER_PEM_CERTIFICATE;
			} 
			
			if (endIndex < 0)
				return null;

			innerContent = contents.Substring(header.Length);
			innerContent = innerContent.Substring(0, endIndex - header.Length);
			innerContent = innerContent.Trim();
			newContent = contents.Substring(endIndex + footer.Length).Trim();

			byte[] ret = null;

			switch (part) {
				case PEMPart.OpenSSLPublic:
					ret = DecodeOpenSSLPublicKey(innerContent);
					break;
				case PEMPart.OpenSSLPrivate:
					ret = DecodeOpenSSLPrivateKey(innerContent, password);
					break;
				case PEMPart.PKCS8:
					ret = DecodePkcs8PrivateKey(innerContent);
					break;
				case PEMPart.PKCS8Encrypted:
					ret = DecodePkcs8EncPrivateKey(innerContent);
					break;
				case PEMPart.Certificate:
					ret = Convert.FromBase64String(innerContent);
					break;
			}
		...
		public static RSACryptoServiceProvider DecodePEMKey(string pemstr, SecureString password, out X509Certificate certificate) {
			certificate = null;
			RSAParameters? partParam = null;
			RSAParameters param = new RSAParameters();
			int index = 0;
			PEMPart part = PEMPart.Unknown;
			byte[] nextPart = null;
			RSACryptoServiceProvider ret = new RSACryptoServiceProvider();

			while ((nextPart = translatePart(pemstr, password, out part, out index, out pemstr)) != null) {
				switch (part) {
					case PEMPart.Certificate:
						//Holds x509 cert
						if ((nextPart != null) && (nextPart.Length > 0) && nextPart[0] == 0x30) {
							certificate = new X509Certificate2(nextPart, password, X509KeyStorageFlags.PersistKeySet);
						}
						break;
					case PEMPart.OpenSSLPublic:
						partParam = DecodeX509PublicKey(nextPart);
						if (partParam != null && partParam.HasValue) {
							param.Modulus = partParam.Value.Modulus;
							param.Exponent = partParam.Value.Exponent;
						}
						break;
					case PEMPart.OpenSSLPrivate:
						partParam = DecodeRSAPrivateKey(nextPart);
						if (partParam != null && partParam.HasValue) {
							param.Modulus	= partParam.Value.Modulus;
							param.Exponent	= partParam.Value.Exponent;
							param.D			= partParam.Value.D;
							param.P			= partParam.Value.P;
							param.Q			= partParam.Value.Q;
							param.DP		= partParam.Value.DP;
							param.DQ		= partParam.Value.DQ;
							param.InverseQ	= partParam.Value.InverseQ;
						}
						break;
					case PEMPart.PKCS8:
						partParam = DecodePrivateKeyInfo(nextPart);
						if (partParam != null && partParam.HasValue) {
							param.Modulus	= partParam.Value.Modulus;
							param.Exponent	= partParam.Value.Exponent;
							param.D			= partParam.Value.D;
						...
						break;
					case PEMPart.PKCS8Encrypted:
						partParam = DecodeEncryptedPrivateKeyInfo(nextPart, password);
						if (partParam != null && partParam.HasValue) {
							param.Modulus	= partParam.Value.Modulus;
							param.Exponent	= partParam.Value.Exponent;
							param.D			= partParam.Value.D;
			...
			ret.ImportParameters(param);
			return ret;
		}

		// ------- Decode PEM public, private or pkcs8 key ----------------
		public static RSACryptoServiceProvider DecodeDERKey(string contents, SecureString password) {
			RSAParameters? partParam = null;
			RSAParameters param = new RSAParameters();
			RSACryptoServiceProvider rsa = new RSACryptoServiceProvider();
			byte[] keyblob = Convert.FromBase64String(contents);
			if (keyblob == null)
				return null;

			if ((partParam = DecodeX509PublicKey(keyblob)) != null && partParam != null && partParam.HasValue) {
				param.Modulus = partParam.Value.Modulus;
				param.Exponent = partParam.Value.Exponent;
			}

			if ((partParam = DecodeRSAPrivateKey(keyblob)) != null && partParam != null && partParam.HasValue) {
				param.Modulus	= partParam.Value.Modulus;
				param.Exponent	= partParam.Value.Exponent;
				param.D			= partParam.Value.D;
				param.P			= partParam.Value.P;
			...
			if ((partParam = DecodePrivateKeyInfo(keyblob)) != null && partParam != null && partParam.HasValue) {
				param.Modulus	= partParam.Value.Modulus;
				param.Exponent	= partParam.Value.Exponent;
				param.D			= partParam.Value.D;
				param.P			= partParam.Value.P;
			...
			if ((partParam = DecodeEncryptedPrivateKeyInfo(keyblob, password)) != null && partParam != null && partParam.HasValue) {
				param.Modulus	= partParam.Value.Modulus;
				param.Exponent	= partParam.Value.Exponent;
				param.D			= partParam.Value.D;
				param.P			= partParam.Value.P;
		...
		public static void ProcessRSA(RSACryptoServiceProvider rsa, SecureString password) {
			if (verbose)
				showRSAProps(rsa);
			Console.Write("\n\nExport RSA private key to PKCS #12 file?  (Y or N) ");
			String resp = Console.ReadLine().ToUpper();
			if (resp == "Y" || resp == "YES")
				RSAtoPKCS12(rsa, password);
		}

		//--------  Generate pkcs #12 from an RSACryptoServiceProvider  ---------
		public static void RSAtoPKCS12(RSACryptoServiceProvider rsa, SecureString ss) {
			...
			byte[] pkcs12 = GetPkcs12(rsa, keycontainer, provider, keyspec, cspflags, ss);
		...
		public static RSAParameters? DecodePrivateKeyInfo(byte[] pkcs8) {
		...
		public static RSAParameters? DecodeEncryptedPrivateKeyInfo(byte[] encpkcs8, SecureString password) {
			// encoded OID sequence for  PKCS #1 rsaEncryption szOID_RSA_RSA = "1.2.840.113549.1.1.1"
			// this byte[] includes the sequence byte and terminal encoded null 
			...
			try {
				...
				SecureString secpswd = password;// GetSecPswd("Enter password for Encrypted PKCS #8 ==>");
		...
		private static int GetIntegerSize(BinaryReader binr) {
			...
			while (binr.PeekChar() == 0x00) {	//remove high order zeros in data
				binr.ReadByte();
				count -= 1;
			}
			return count;
		...
		public static byte[] DecodeOpenSSLPrivateKey(string pvkstr, SecureString password) {
			byte[] binkey = null;
		...
		private static byte[] GetPkcs12(RSA rsa, string keycontainer, string cspprovider, uint KEYSPEC, uint cspflags, SecureString password) {
			byte[] pfxblob = null;
			...
			try {
				...
				SecureString pswd = password;// GetSecPswd("Set PFX Password ==>");
		...
		private static SecureString convertToSecure(string val) {
			if (val == null)
		...
		public static ParseResult Parse(CertificateFormat Format, string FileName) { 
			return Parse(Format, FileName, null); 
		}

		public static ParseResult Parse(CertificateFormat Format, Stream Stream, string Password) {
			return Parse(Format, Stream, convertToSecure(Password));
		}

		public static ParseResult ParseDefault(byte[] Contents, SecureString Password) {
			return new ParseResult(new X509Certificate(Contents, Password), null);
		}

		public static ParseResult ParseDefault(byte[] Contents, string Password) {
			return new ParseResult(new X509Certificate(Contents, Password), null);
		}

		public static ParseResult ParseContents(CertificateFormat Format, string Contents, string Password) {
			return ParseContents(Format, Contents, convertToSecure(Password));
		}

		public static ParseResult Parse(CertificateFormat Format, Stream Stream, SecureString Password) {
			StreamReader sr = new StreamReader(Stream);
			string contents = sr.ReadToEnd();
			sr.Close();
			sr.Dispose();

			return ParseContents(Format, contents, Password);
		}

		public static ParseResult Parse(CertificateFormat Format, string FileName, string Password) {
			if (string.IsNullOrEmpty(FileName))
				throw new NullReferenceException("FileName is empty or null");
			if (!File.Exists(FileName))
				throw new FileNotFoundException("File not found", FileName);

			if (Format != CertificateFormat.Default)
				return ParseContents(Format, File.ReadAllText(FileName), Password);
			else
				return ParseDefault(File.ReadAllBytes(FileName), Password);
		}

		public static ParseResult ParseContents(CertificateFormat Format, string Contents, SecureString Password) {
			RSACryptoServiceProvider provider = null;
			X509Certificate cert = null;

			switch (Format) {
				case CertificateFormat.PEM:
					provider = DecodePEMKey(Contents, Password, out cert);
					break;
				case CertificateFormat.DER:
					provider = DecodeDERKey(Contents, Password);
					break;
			}

			return new ParseResult(cert, provider);
		}
		#endregion
		...
		public class ParseResult {
			...
			internal ParseResult(X509Certificate certificate, RSACryptoServiceProvider provider) {
