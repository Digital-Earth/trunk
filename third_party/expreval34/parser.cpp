...
Node *Parser::ParseRegion(Parser::size_type start, Parser::size_type end)
	...
	size_type comparissionindex = (size_type)-1;
	size_type logicalAndIndex = (size_type)-1;
	size_type logicalOrIndex = (size_type)-1;
                ...
                if(plevel == 0)
                    ...
                    if(pos == start)
                        ...
                        switch(m_tokens[pos - 1].GetType())
							...
							case Token::TypeSmaller:
							case Token::TypeGreater:
							case Token::TypeSmallerEqual:
							case Token::TypeGreaterEqual:
							case Token::TypeLogicalAnd:
							case Token::TypeLogicalOr:
	...
	else if (logicalOrIndex != (size_type)-1)
	{
		auto_ptr<Node> n(new LogicalOrNode(m_expr));
        n->Parse(*this, start, end, logicalOrIndex);
        return n.release();
	}
	else if (logicalAndIndex != (size_type)-1)
	{
		auto_ptr<Node> n(new LogicalAndNode(m_expr));
        n->Parse(*this, start, end, logicalAndIndex);
        return n.release();
	}
	else if(comparissionindex != (size_type)-1)
	{
		//Comparssion operator
		if(m_tokens[comparissionindex].GetType() == Token::TypeEquals)
		{
			auto_ptr<Node> n(new CompareEqualNode(m_expr));
            n->Parse(*this, start, end, comparissionindex);
            return n.release();
		}
		else if(m_tokens[comparissionindex].GetType() == Token::TypeSmaller)
		{
			auto_ptr<Node> n(new CompareSmallerNode(m_expr));
            n->Parse(*this, start, end, comparissionindex);
            return n.release();
		}
		else if(m_tokens[comparissionindex].GetType() == Token::TypeGreater)
		{
			auto_ptr<Node> n(new CompareGreaterNode(m_expr));
            n->Parse(*this, start, end, comparissionindex);
            return n.release();
		}
		else if(m_tokens[comparissionindex].GetType() == Token::TypeSmallerEqual)
		{
			auto_ptr<Node> n(new CompareSmallerEqualNode(m_expr));
            n->Parse(*this, start, end, comparissionindex);
            return n.release();
		}
		else if(m_tokens[comparissionindex].GetType() == Token::TypeGreaterEqual)
		{
			auto_ptr<Node> n(new CompareGreaterEqualNode(m_expr));
            n->Parse(*this, start, end, comparissionindex);
            return n.release();
		}
		else {
			...
			e.SetStart(m_tokens[comparissionindex].GetStart());
			e.SetEnd(m_tokens[comparissionindex].GetEnd());
			throw(e);
		}
	}
        ...
        switch(exstr[pos])
                ...
                if(comment == false)
                    {
						if (exstr[pos+1] == '=')
						{
							m_tokens.push_back(Token(Token::TypeEquals, pos, pos+1));
							pos++;
						}
						else 
						{
			...
			case '<':
				if(comment == false)
                {
					if (exstr[pos+1] == '=')
					{
						m_tokens.push_back(Token(Token::TypeSmallerEqual, pos, pos+1));
						pos++;
					}
					else 
					{
						m_tokens.push_back(Token(Token::TypeSmaller, pos, pos));
					}
				}
				break;
			case '>':
				if(comment == false)
                {
					if (exstr[pos+1] == '=')
					{
						m_tokens.push_back(Token(Token::TypeGreaterEqual, pos, pos+1));
						pos++;
					}
					else 
					{
						m_tokens.push_back(Token(Token::TypeGreater, pos, pos));
					}
				}
				break;
            ...
            case '&':
                ...
                if(comment == false)
                {
					if (exstr[pos+1] == '&')
					{
						m_tokens.push_back(Token(Token::TypeLogicalAnd, pos, pos+1));
						pos++;
					}
					else 
					{
            ...
            case '|':
                ...
                if(comment == false)
				{
					if (exstr[pos+1] == '|')
					{
						m_tokens.push_back(Token(Token::TypeLogicalOr, pos, pos+1));
						pos++;
					}
					else 
                    {
						m_tokens.push_back(Token(Token::TypeLogicalOr, pos, pos));
                    }
				}
                break;
                }
