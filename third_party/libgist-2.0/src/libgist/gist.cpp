...
// $Id: gist.cpp,v 1.3 2006/09/06 13:42:34 syarrow Exp $
...
gist::open(
    const char*		filename)
{
    W_DO(_file.open(filename, const_cast<const gist_ext_t *&>( _ext)));
    _isOpen = true;
    return(RCOK);
}
    ...
    const int	    	dlen)
{
	...
	/*
	Removed following assert.  If function is_empty is called it will pin a page thereby
	causing this assert to fail.
	*/
    // XXX debug
